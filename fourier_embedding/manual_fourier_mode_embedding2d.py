...
def plot_err(epoch,model_sines):
    ...
    err_test = o_test_grid/MAX_o_train-pred_test_grid
    plot.figure(epoch)
    plot.title('Full Resolution')
    plot.subplot(3,1,1)
    plot.contourf(X_grid,Y_grid,o_test_grid/MAX_o_train,levels=21)
    plot.set_cmap('bwr')
    plot.colorbar()
    plot.subplot(3,1,2)
...
global o_test_grid
o_test_grid = np.reshape(phi_xr,X_grid.shape)
MAX_x = np.max(X_grid)
MAX_y = np.max(Y_grid)
...
o_train_grid  = np.reshape(phi_xr,(n_x_d,n_y_d))
...
if True:
    ...
    with tf.device('/CPU:0'):
        model_sines = keras.Sequential()
        model_sines.add(keras.layers.Dense(2,activation='linear',input_shape=(2,)))
        model_sines.add(AdjustableFourierTransformLayer(120,30))
        model_sines.add(keras.layers.Dense(nodes,activation='linear'))
        model_sines.add(ResidualLayer(nodes))
        model_sines.add(ResidualLayer(nodes))
        model_sines.add(ResidualLayer(nodes))
...
err_train = o_train_grid/MAX_o_train - pred_train_grid
...
plot.title('S=8')
plot.subplot(3,1,1)
plot.contourf(X_train_grid,Y_train_grid,o_train_grid/MAX_o_train,levels=21)
plot.set_cmap('bwr')
plot.colorbar()
plot.subplot(3,1,2)
