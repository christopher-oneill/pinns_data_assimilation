...
for c in range(len(rec_mode_vec)):
    ...
    for sn in range(len(snapshots)):
        ...
        ux_plot = ax.contourf(X_grid,Y_grid,ux_err,levels=levels_ux_err,cmap= matplotlib.colormaps['bwr'],extend='both')
        ax.set_aspect('equal')
        ax.set_ylabel('y/D',fontsize=5)
        ax.yaxis.set_tick_params(labelsize=5)
        ax.xaxis.set_tick_params(labelbottom=False)
        ...
        cbar = plot.colorbar(ux_plot,cax,ticks=[MAX_ux_err,MAX_ux_err/2,0.0,-MAX_ux_err/2,-MAX_ux_err],format=tkr.FormatStrFormatter('%.2f'))
        ticklabs = cbar.ax.get_yticklabels()
        cbar.ax.set_yticklabels(ticklabs, fontsize=5)
        fig.add_subplot(cax)
        ...
        uy_plot = ax.contourf(X_grid,Y_grid,uy_err,levels=levels_uy_err,cmap= matplotlib.colormaps['bwr'],extend='both')
        ax.set_aspect('equal')
        ax.set_yticks(np.array([2.0,1.0,0.0,-1.0,-2.0]))
        ax.xaxis.set_tick_params(labelbottom=False)
        ax.yaxis.set_tick_params(labelleft=False)
        ...
        cbar = plot.colorbar(uy_plot,cax,ticks=[MAX_uy_err,MAX_uy_err/2,0.0,-MAX_uy_err/2,-MAX_uy_err],format=tkr.FormatStrFormatter('%.2f'))
        ticklabs = cbar.ax.get_yticklabels()
        cbar.ax.set_yticklabels(ticklabs, fontsize=5)
        fig.add_subplot(cax)
        ...
        p_plot = ax.contourf(X_grid,Y_grid,p_err,levels=levels_p_err,cmap= matplotlib.colormaps['bwr'],extend='both')
        ax.set_aspect('equal')
        ax.yaxis.set_tick_params(labelsize=5)
        ax.set_ylabel('y/D',fontsize=5)
        ...
        cbar = plot.colorbar(p_plot,cax,ticks=[-MAX_p_err,-MAX_p_err/2,0.0,MAX_p_err/2.0,MAX_p_err],format=tkr.FormatStrFormatter('%.2f'))
        ticklabs = cbar.ax.get_yticklabels()
        cbar.ax.set_yticklabels(ticklabs, fontsize=5)
        fig.add_subplot(cax)
        ...
        p_plot = ax.contourf(X_grid,Y_grid,vorticity_err,levels=levels_vorticity,cmap= matplotlib.colormaps['bwr'],extend='both')
        ax.set_aspect('equal')
        ax.yaxis.set_tick_params(labelsize=5)
        ax.xaxis.set_tick_params(labelsize=5)
        ax.yaxis.set_tick_params(labelleft=False)
        ...
        plot.savefig(figures_dir+'linerr_snapshot'+str(snap_ind)+'_contours_ref_recref_c'+str(rec_mode_vec[c])+'.pdf')
        plot.close(fig)
...
for c in range(len(rec_mode_vec)):
    ...
    for s in range(len(cases_supersample_factor)):
        ...
        for sn in range(len(snapshots)):
            ...
            cax=plot.Subplot(fig,inner[2][7])
            cbar = plot.colorbar(p_plot,cax,ticks=[MAX_p_err,MAX_p_err/2,0.0,-MAX_p_err/2,-MAX_p_err],format=tkr.FormatStrFormatter('%.2f'))
            ticklabs = cbar.ax.get_yticklabels()
            cbar.ax.set_yticklabels(ticklabs, fontsize=5)
            fig.add_subplot(cax)
            ...
            plot.savefig(figures_dir+'linerr_snapshot'+str(snapshots[sn])+'_contours_ref_pinn_S'+str(cases_supersample_factor[s])+'_c'+str(rec_mode_vec[c])+'.pdf')
            plot.close(fig)
            ...
            plot.savefig(figures_dir+'linerr_snapshot'+str(snapshots[sn])+'_contours_recref_pinn_S'+str(cases_supersample_factor[s])+'_c'+str(rec_mode_vec[c])+'.pdf')
            plot.close(fig)