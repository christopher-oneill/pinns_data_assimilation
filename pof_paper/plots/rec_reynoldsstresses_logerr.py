...
k_ref = 0.5*np.mean(np.power(ux_ref[:,0:L_dft]+np.reshape(ux,[ux.shape[0],1]),2.0)+np.power(uy_ref[:,0:L_dft]+np.reshape(uy,[uy.shape[0],1]),2.0),axis=1)
k_ref = np.reshape(k_ref,[X_grid.shape[0],X_grid.shape[1]])
...
for c in rec_mode_vec:
    ...
    uxux_rec_ref[c][cylinder_mask]=np.NaN
    uxux_rec_ref[c][cylinder_mask]=np.NaN
    k_rec_ref.append(0.5*np.mean(np.power(ux_rec_ref,2.0)+np.power(uy_rec_ref,2.0),axis=2))
    k_rec_ref[c][cylinder_mask]=np.NaN
