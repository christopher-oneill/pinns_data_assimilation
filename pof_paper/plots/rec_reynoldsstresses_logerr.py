...
if True:
    ...
    c=2
    s=4
    uxux_err_grid1 = uxux_rec[s][c] - uxux_ref
    uxuy_err_grid1 = uxuy_rec[s][c] - uxuy_ref
    uyuy_err_grid1 = uyuy_rec[s][c] - uyuy_ref
    ...
    # grid the data
    s=5
    uxux_err_grid2 = uxux_rec[s][c] - uxux_ref
    uxuy_err_grid2 = uxuy_rec[s][c] - uxuy_ref
    uyuy_err_grid2 = uyuy_rec[s][c] - uyuy_ref
    ...
    plot.savefig(figures_dir+'logerr_reynoldsStress_contours_ref_pinn_condensed_c2.pdf')
    plot.savefig(figures_dir+'logerr_reynoldsStress_contours_ref_pinn_condensed_c2.png',dpi=300)
    plot.close(fig)
...
for s in range(len(cases_supersample_factor)):
    ...
    for c in rec_mode_vec:
        err_uxux = (np.abs(uxux_rec[s][c]-uxux_ref)/MAX_uxux_ref).ravel()
        err_uxuy = (np.abs(uxuy_rec[s][c]-uxuy_ref)/MAX_uxuy_ref).ravel()
        err_uyuy = (np.abs(uyuy_rec[s][c]-uyuy_ref)/MAX_uyuy_ref).ravel()
        ...
        err_uxux_FMD = (np.abs(uxux_rec_ref[c]-uxux_ref)/MAX_uxux_ref).ravel()
        err_uxuy_FMD = (np.abs(uxuy_rec_ref[c]-uxuy_ref)/MAX_uxuy_ref).ravel()
        err_uyuy_FMD = (np.abs(uyuy_rec_ref[c]-uyuy_ref)/MAX_uyuy_ref).ravel()
        ...
        err_uxux_PINN_FMD = (np.abs(uxux_rec[s][c]-uxux_rec_ref[c])/MAX_uxux_ref).ravel()
        err_uxuy_PINN_FMD = (np.abs(uxuy_rec[s][c]-uxuy_rec_ref[c])/MAX_uxuy_ref).ravel()
        err_uyuy_PINN_FMD = (np.abs(uyuy_rec[s][c]-uyuy_rec_ref[c])/MAX_uyuy_ref).ravel()
...
for c in [0,1,2]:
    ...
    #fig.tight_layout()
    plot.savefig(figures_dir+'logerr_rec_reynoldsStress_f'+str(c)+'_error_FMD.pdf')
    plot.savefig(figures_dir+'logerr_rec_reynoldsStress_f'+str(c)+'_error_FMD.png',dpi=300)
    plot.close(fig)
