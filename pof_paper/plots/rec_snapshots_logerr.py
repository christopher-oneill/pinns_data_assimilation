...
if False:
    ...
    for c in range(len(rec_mode_vec)):
        ...
        for sn in range(len(snapshots)):
            ...
            vorticity_err = (vorticity_ref[:,:,snap_ind]-vorticity_FMD[c][:,:,snap_ind])
            ...
            ax.xaxis.set_tick_params(labelbottom=False)
            ax.yaxis.set_tick_params(labelleft=False)
            ax.text(7,1.5,'$\omega_{DNS}$',fontsize=8)
            ax.text(-1.75,1.4,'(da)',fontsize=8,color='k')
            circle = plot.Circle((0,0),0.5,color='k',fill=False,zorder=2)
            ax.add_patch(circle)
            fig.add_subplot(ax)
            ...
            p_plot =ax.contourf(X_grid,Y_grid,vorticity_FMD[c][:,:,snap_ind],levels=levels_vorticity,cmap= matplotlib.colormaps['bwr'],extend='both')
            ax.set_aspect('equal')
            ax.yaxis.set_tick_params(labelsize=8)
            ax.xaxis.set_tick_params(labelbottom=False)
            ax.yaxis.set_tick_params(labelleft=False)
            ax.text(7,1.5,'$\omega_{FMD}$',fontsize=8)
            ax.text(-1.75,1.4,'(db)',fontsize=8,color='k')
            circle = plot.Circle((0,0),0.5,color='k',fill=False,zorder=2)
            ax.add_patch(circle)
            fig.add_subplot(ax)
            ...
            circle = plot.Circle((0,0),0.5,color='k',fill=False,zorder=2)
            ax.add_patch(circle)
            ax.text(7,1.5,'$|\omega_{DNS}-\omega_{FMD}|$',fontsize=8,color='k')
            ax.text(-1.75,1.4,'(dc)',fontsize=8,color='k')
            fig.add_subplot(ax)
    ...
    for c in range(len(rec_mode_vec)):
        ...
        for s in range(len(cases_supersample_factor)):
            ...
            for sn in range(len(snapshots)):
                ...
                MAX_vorticity_FMD = np.nanmax(np.abs(vorticity_FMD[c].ravel()))
                ...
                if False:
                    ...
                    ax.xaxis.set_tick_params(labelbottom=False)
                    ax.yaxis.set_tick_params(labelleft=False)
                    ax.text(7,1.5,'$\omega_{PINN}$',fontsize=8)
                    ax.text(-1.75,1.4,'(db)',fontsize=8,color='k')
                    ...
                    ax.yaxis.set_tick_params(labelleft=False)
                    ax.set_xlabel('x/D',fontsize=8)
                    ax.text(7,1.5,'$|\omega_{DNS}-\omega_{PINN}|$',fontsize=8,color='k')
                    ax.text(-1.75,1.4,'(dc)',fontsize=8,color='k')
                    circle = plot.Circle((0,0),0.5,color='k',fill=False,zorder=2)
                    ax.add_patch(circle)
                    fig.add_subplot(ax)
                ...
                vorticity_err = (vorticity_FMD[c][:,:,snap_ind]-vorticity_rec[s][c][:,:,snap_ind])
                ...
                if False:
                    # quadrant 4, plot the vorticity (unused)
                    ...
                    ax.yaxis.set_tick_params(labelleft=False)
                    ax.set_xlabel('x/D',fontsize=8)
                    ax.text(7,1.5,'$|\omega_{FMD}-\omega_{PINN}|$',fontsize=8,color='k')
                    ax.text(-1.75,1.4,'(dc)',fontsize=8,color='k')
                    circle = plot.Circle((0,0),0.5,color='k',fill=False,zorder=2)
                    ax.add_patch(circle)
                    fig.add_subplot(ax)
