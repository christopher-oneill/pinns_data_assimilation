...
case = 'JFM'
save_loc = './data/limacher/tmp/checkpoint'
checkpoint_filepath = './data/limacher/tmp/checkpoint'
...
# read the data
meanfilename = 'C:/projects/pinns_galerkin_viv/data/limacher/meanField.mat'
meanFieldFile = h5py.File(meanfilename,'r')
configfilename = 'C:/projects/pinns_galerkin_viv/data/limacher/configuration.mat'
configFile = h5py.File(configfilename)

ux = np.array(meanFieldFile['meanField'][0,:,:])
uy = np.array(meanFieldFile['meanField'][1,:,:])
x = np.array(configFile['X'])
y = np.array(configFile['Y'])
...
x_colloc = x[np.power(np.power(x,2)+np.power(y,2),0.5)>(0.5*d)]
y_colloc = y[np.power(np.power(x,2)+np.power(y,2),0.5)>(0.5*d)]
# reduce the collocation points to 25k
colloc_rand = np.random.choice(x_colloc.size,2500);
x_colloc = x_colloc[colloc_rand]
y_colloc = y_colloc[colloc_rand]
...
def custom_loss_wrapper(colloc_tensor_f,BCs_p): # def custom_loss_wrapper(colloc_tensor_f,BCs,BCs_p,BCs_t):
    ...
    def custom_loss(y_true, y_pred):
        ...
        return data_loss1 + data_loss2  + 1*physical_loss1 + 1*physical_loss2 + 1*physical_loss3 + 1*f_boundary_p # + f_boundary_t1+ f_boundary_t2 
...
model.save_weights('./tmp/'+model_structure_string+'b64_2')
