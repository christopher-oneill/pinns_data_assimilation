...
if True:
    # save the mode max magnitudes
    u_modes = np.array(fourierModeFile['velocityModes'])
    p_modes = (fourierModeFile['pressureModes'])


    err_txt_file = open(figures_dir+'mode_magnitudes.txt','w')

    err_txt_file.write('Mean field magnitudes \n')

    err_txt_file.write('\nMax abs ux: ')
    err_txt_file.write(str(np.max(np.abs(ux),axis=0)))
    err_txt_file.write('\nMax abs uy: ')
    err_txt_file.write(str(np.max(np.abs(uy),axis=0)))
    err_txt_file.write('\nMax abs p: ')
    err_txt_file.write(str(np.max(np.abs(p),axis=0)))

    err_txt_file.write('Fourier mode magnitudes \n')

    temp_max_ux = np.max(np.abs(u_modes[:,:,0]),axis=0)
    temp_max_uy = np.max(np.abs(u_modes[:,:,1]),axis=0)
    temp_max_p = np.max(np.abs(p_modes),axis=0)

    modes = np.arange(1,9)

    err_txt_file.write('Modes :')
    err_txt_file.write(str(modes))
    err_txt_file.write('\nMax abs phi_x: ')
    err_txt_file.write(str(temp_max_ux))
    err_txt_file.write('\nMax abs phi_y:')
    err_txt_file.write(str(temp_max_uy))
    err_txt_file.write('\nMax abs psi:')
    err_txt_file.write(str(temp_max_p))
...
if True:
    ...
    fig = plot.figure(figsize=(5.85,2.5))
    plot.subplots_adjust(left=0.04,top=0.99,right=0.98,bottom=0.15)
    outer = gridspec.GridSpec(1,2,wspace=0.3,hspace=0.12)

    ax = plot.Subplot(fig,outer[0])

    uxuy_plot, =ax.plot(f_ux_modes,np.abs(ux_modes[1,:])*shift,linewidth=line_width,linestyle='-',color='k')
    ax.plot(f_ux_modes[ux_mode_inds],np.abs(ux_modes[1,ux_mode_inds])*shift,linewidth=0.0,marker='o',color='k',markersize=1.0)
    ax.plot(f_ux_modes,np.abs(uy_modes[1,:]),linewidth=line_width,linestyle='-',color='k')
    ax.plot(f_ux_modes[ux_mode_inds],np.abs(uy_modes[1,ux_mode_inds]),linewidth=0.0,marker='o',color='k',markersize=1.0)
    ax.plot(f_ux_modes,np.abs(p_modes[1,:])/shift,linewidth=line_width,linestyle='-',color='k')
    ax.plot(f_ux_modes[ux_mode_inds],np.abs(p_modes[1,ux_mode_inds])/shift,linewidth=0.0,marker='o',color='k',markersize=1.0)

    selected_modes_f = f_ux_modes[ux_mode_inds]
    selected_modes_a = np.abs(ux_modes[1,ux_mode_inds])*shift
    for m in range(selected_modes_f.shape[0]):
        ax.text(selected_modes_f[m]+0.05,0.5*selected_modes_a[m],str(m+1),fontsize=8)

    ax.text(1.6,np.abs(ux_modes[1,1]*shift*1E1),'$\Phi_x$',fontsize=8)
    ax.text(1.6,np.abs(uy_modes[1,1])*1E1,'$\Phi_y$',fontsize=8)
    ax.text(1.6,np.abs(p_modes[1,1]/shift)*1E1,'$\psi$',fontsize=8)

    ax.yaxis.set_tick_params(labelleft=False)
    ax.yaxis.set_tick_params(left=False)
    ax.set_ylabel('$|\psi|, |\Phi_y|, |\Phi_x|$',fontsize=8)
    ax.set_yscale('log')
    ax.set_xlim(0,1.7)
    ax.set_ylim(1E-10,1E4)
    ax.set_xticks(x_ticks)
    ax.set_xticklabels(x_tick_labels,fontsize=8)
    ax.set_xlabel('Frequency (St)',fontsize=8)
    ax.text(0.04,0.5*np.abs(ux_modes[1,ux_mode_inds[0]])*shift*1E0,'(a)',fontsize=8,color='k')
    ...
    mean_plt_ux,=ax.plot(plot_mode_vec-0.1,mean_err_ux_p,linewidth=0,marker='o',color='blue',markersize=3,markerfacecolor='blue')
    max_plt_ux,=ax.plot(plot_mode_vec-0.1,max_err_ux_p,linewidth=0,marker='v',color='blue',markersize=3,markerfacecolor='blue')
    mean_plt_uy,=ax.plot(plot_mode_vec,mean_err_uy_p,linewidth=0,marker='o',color='red',markersize=3,markerfacecolor='red')
    max_plt_uy,=ax.plot(plot_mode_vec,max_err_uy_p,linewidth=0,marker='v',color='red',markersize=3,markerfacecolor='red')
    mean_plt_p,=ax.plot(plot_mode_vec+0.1,mean_err_p_p,linewidth=0,marker='o',color='green',markersize=3,markerfacecolor='green')
    max_plt_p,=ax.plot(plot_mode_vec+0.1,max_err_p_p,linewidth=0,marker='v',color='green',markersize=3,markerfacecolor='green')
    ax.set_xticks(plot_mode_vec)
    ...
    ax.legend([mean_plt_ux,max_plt_ux,mean_plt_uy,max_plt_uy,mean_plt_p,max_plt_p],["Mean $u'_x$","Max $u'_x$","Mean $u'_y$","Max $u'_y$","Mean $p'$","Max $p'$",],fontsize=8,ncols=2)
    ax.grid('on')
    ax.set_xlabel('Number of Fourier Modes',fontsize=8)
    ax.text(1,3.0,'(b)',fontsize=8,color='k')
    fig.add_subplot(ax)


    plot.savefig(figures_dir+'truncation_combined.pdf')
    plot.savefig(figures_dir+'truncation_combined.png',dpi=600)
    plot.close(fig)       

err_txt_file = open(figures_dir+'truncation_errors.txt','w')
...
err_txt_file.write('\nMean err ux: ')
err_txt_file.write(str(np.array(mean_err_ux_p)))
err_txt_file.write('\nMax err ux:')
err_txt_file.write(str(np.array(max_err_ux_p)))
err_txt_file.write('\nMean err uy:')
err_txt_file.write(str(np.array(mean_err_uy_p)))
err_txt_file.write('\nMax err uy:')
err_txt_file.write(str(np.array(max_err_uy_p)))
err_txt_file.write('\nMean err p:')
err_txt_file.write(str(np.array(mean_err_p_p)))
err_txt_file.write('\nMax err p:')
err_txt_file.write(str(np.array(max_err_p_p)))
err_txt_file.write('\n')
