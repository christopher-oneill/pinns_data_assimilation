...
figures_dir = 'F:/projects/paper_figures/t010_f2/energy_exchange/'
...
for ik in range(len(cases_supersample_factor)):
    ...
    cases_list_m.append('mfg_t010_001_S'+str(cases_supersample_factor[ik])+'/mfg_t010_001_S'+str(cases_supersample_factor[ik])+'_ep'+str(file_number)+'_pred.mat')
...
for s in range(len(cases_supersample_factor)):
    ...
    for c in [0,1,2,3,4,5]:
        # the c here corresponds to the index in the list of files
    ...
    for c in [0,1,2,3,4,5]:
        ...
        # the c here corrsponds to the index in the array of phi_pred, so its +1 in the brackets
        phi_x_pred[s].append(np.conj(phi_x_pred[s][c+1]))
        phi_y_pred[s].append(np.conj(phi_y_pred[s][c+1]))
...
if True:
    ...
    # combined plot with both DS and PINN
        ...
        # make the mode energy crossplot
        fig = plot.figure(figsize=(3.37,4))
        plot.subplots_adjust(left=0.1,top=0.99,right=0.85,bottom=0.07)
        outer = gridspec.GridSpec(3,1,wspace=0.05,hspace=0.1,height_ratios=[1,1,1])
        inner = []
        ...
        filename = 'simplified_energy_exchange_error_ds_PINN_S'+str(supersample_factors_ds[s])+'_short'
        plot.savefig(figures_dir+filename+'.pdf')
        plot.savefig(figures_dir+filename+'.png',dpi=300)
        plot.close(fig)
    ...
    y_ticks=[1E-5,1E-4,1E-3,1E-2,1E-1,1,10]
    y_tick_labels=['1E-5','1E-4','1E-3','0.01','0.1','1','10']
    error_x_tick_labels = ['40','20','10','5','2.5','1.25',]
    ...
    error_file = open(figures_dir+'energy_error.txt','w')
    error_file.write('Downsampled Density:\n')
    error_file.write(str(pts_per_d[1:])+'\n')
    error_file.write('Mean DS Error\n')
    error_file.write(str(mean_error_ds)+'\n')
    error_file.write('Max DS Error\n')
    error_file.write(str(max_error_ds)+'\n')
    error_file.write('PINN Density:\n')
    error_file.write(str(pts_per_d[:])+'\n')
    error_file.write('Mean PINN Error\n')
    error_file.write(str(mean_error_PINN)+'\n')
    error_file.write('Max PINN Error\n')
    error_file.write(str(max_error_PINN)+'\n')
    ...
    if True:
        s =len(supersample_factors_ds)-3 # S*=8
               
        # make the mode energy crossplot
        fig = plot.figure(figsize=(5.85,3.5))
        plot.subplots_adjust(left=0.05,top=0.99,right=0.98,bottom=0.12)
        outer = gridspec.GridSpec(1,2,wspace=0.5,hspace=0.1,width_ratios=[0.55,0.45])
        mid = []
        mid.append(gridspec.GridSpecFromSubplotSpec(3,1,subplot_spec=outer[0],wspace=0.05,hspace=0.05,height_ratios=[1,1,1]))
        inner = []
        ...
        inner.append(gridspec.GridSpecFromSubplotSpec(1,3,subplot_spec=mid[0][0],wspace=0.05,hspace=0.1,width_ratios=[0.48,0.48,0.04]))
        ax = plot.Subplot(fig,inner[0][0])
        ax.set_aspect('equal')
        ...
        # err DS
        inner.append(gridspec.GridSpecFromSubplotSpec(1,3,subplot_spec=mid[0][1],wspace=0.05,hspace=0.1,width_ratios=[0.48,0.48,0.04]))
        ax = plot.Subplot(fig,inner[1][0])
        ax.set_aspect('equal')
        ...
        # err PINN
        inner.append(gridspec.GridSpecFromSubplotSpec(1,3,subplot_spec=mid[0][2],wspace=0.05,hspace=0.1,width_ratios=[0.48,0.48,0.04]))
        ax = plot.Subplot(fig,inner[2][0])
        ax.set_aspect('equal')
        ...
        # error
        mid.append(gridspec.GridSpecFromSubplotSpec(2,1,subplot_spec=outer[1],wspace=0.02,hspace=0.1,height_ratios=[0.15,0.85,]))

        ax = plot.Subplot(fig,mid[1][1])
        y_ticks=[1E-5,1E-4,1E-3,1E-2,1E-1,1,10]
        y_tick_labels=['1E-5','1E-4','1E-3','0.01','0.1','1','10']
        error_x_tick_labels = ['40','20','10','5','2.5','1.25',]

        mean_plt,=ax.plot(pts_per_d[1:]*0.97,mean_error_ds,linewidth=0,marker='o',color='blue',markersize=3,markerfacecolor='blue')
        mean_plt2,=ax.plot(pts_per_d*1.03,mean_error_PINN,linewidth=0,marker='o',color='red',markersize=3,markerfacecolor='red')
        max_plt,=ax.plot(pts_per_d[1:]*0.97,max_error_ds,linewidth=0,marker='v',color='blue',markersize=3,markerfacecolor='blue')
        max_plt2,=ax.plot(pts_per_d*1.03,max_error_PINN,linewidth=0,marker='v',color='red',markersize=3,markerfacecolor='red')
        ax.set_xscale('log')
        ax.xaxis.set_tick_params(labelbottom=True)
        ax.set_yscale('log')
        ax.set_ylim(1E-5,1E1)
        ax.set_yticks(y_ticks,labels=y_tick_labels,fontsize=8)
        ax.set_ylabel("Error ($\eta$)",fontsize=8)
        #ax.set_title('$\overline{u\'_{x}u\'_{x}}$')
        ax.legend([mean_plt,mean_plt2,max_plt,max_plt2],['Mean DS','Mean PINN','Max DS','Max PINN'],fontsize=8,ncol=2,bbox_to_anchor=(1.05, 1.2))
        ax.grid('on')
        ax.set_xticks(pts_per_d)
        ax.set_xticklabels(error_x_tick_labels)
        ax.set_xlabel('$D/\Delta x$',fontsize=8)
        ax.text(1.5,5.0,'(g)',fontsize=8)
        ...
        # now with D/x=10
        #s =len(supersample_factors_ds)-2-1 # S*=8
        s =len(supersample_factors_ds)-3-1 # S*=4 D/x=10       
        # make the mode energy crossplot
        fig = plot.figure(figsize=(5.85,3.5))
        plot.subplots_adjust(left=0.05,top=0.99,right=0.98,bottom=0.12)
        outer = gridspec.GridSpec(1,2,wspace=0.5,hspace=0.1,width_ratios=[0.55,0.45])
