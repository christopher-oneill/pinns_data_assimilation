...
figures_dir = 'F:/projects/paper_figures/t010/rec_snapshots/'
...
if True:
    ...
    plot.savefig(figures_dir+'c5/logerr_snapshot'+str(snapshots[sn])+'_contours_ref_pinn_S8S32_c5_condensed.pdf')
    plot.savefig(figures_dir+'c5/logerr_snapshot'+str(snapshots[sn])+'_contours_ref_pinn_S8S32_c5_condensed.png',dpi=300)
    plot.close(fig)
...
err_file = open(figures_dir+'Err_Rec_Snapshots.txt','w')


for c in [5]:

    err_file.write('FMD truncation ')
    err_file.write(str(c))
    err_file.write('\nerror relative to DNS: ')
    err_file.write('\nmean ux: ')
    err_file.write(str(mean_err_ux[:,c]))
    err_file.write('\nmax ux: ')
    err_file.write(str(max_err_ux[:,c]))
    err_file.write('\nmean uy: ')
    err_file.write(str(mean_err_uy[:,c]))
    err_file.write('\nmax uy:')
    err_file.write(str(max_err_uy[:,c]))
    err_file.write('\nmean p: ')
    err_file.write(str(mean_err_p[:,c]))
    err_file.write('\nmax p: ')
    err_file.write(str(max_err_p[:,c]))
    err_file.write('\n\nerror relative to FMD:')
    err_file.write('\nmean ux: ')
    err_file.write(str(mean_err_ux_PINN_FMD[:,c]))
    err_file.write('\nmax ux: ')
    err_file.write(str(max_err_ux_PINN_FMD[:,c]))
    err_file.write('\nmean uy: ')
    err_file.write(str(mean_err_uy_PINN_FMD[:,c]))
    err_file.write('\nmax uy: ')
    err_file.write(str(max_err_uy_PINN_FMD[:,c]))
    err_file.write('\nmean p: ')
    err_file.write(str(mean_err_p_PINN_FMD[:,c]))
    err_file.write('\nmax p: ')
    err_file.write(str(max_err_p_PINN_FMD[:,c]))
...
if True:
    for c in range(len(rec_mode_vec)):
        ...
        plot.subplots_adjust(left=0.2,top=0.95,right=0.97,bottom=0.13)
