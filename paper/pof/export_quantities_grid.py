...
input_data_dir = 'I:/projects/fixed_cylinder/grid/data/'
output_data_dir = 'C:/projects/pinns_narval/sync/data/mazi_fixed_grid/'
...
inputMeanPressureFile = h5py.File(input_data_dir+'meanPressure.mat','r')
velocity_file = h5py.File(input_data_dir+'fluctuatingVelocity.mat','r')
pressure_file = h5py.File(input_data_dir+'fluctuatingPressure.mat','r')
...
inputMeanPressureField = np.array(inputMeanPressureFile['meanPressure']).transpose()
inputFluctuatingPressureField = np.array(pressure_file['fluctuatingPressure']).transpose()
...
pressureField = inputFluctuatingPressureField + np.reshape(inputMeanPressureField,[inputFluctuatingPressureField.shape[0],1])
# truncate per L_dft
...
reynoldsStress = np.reshape(reynoldsStress,[fluctuatingVelocity.shape[0],3]).transpose()
