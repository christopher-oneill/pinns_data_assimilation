...
case_scripts_folder = run_script_folder + 't10_f/'

cluster_case_file_loc = "/home/coneill/sync/code/pinns_data_assimilation/slurm/mazi_fixed_grid/test/t10_f/"

frequencies = [0,1,2,3,4,5]
supersample_factors = [0,2,4,8,16,32]

run_file = open(run_script_folder+'run_t10_f.sh','w')
run_line1 = "#!/bin/bash\n\n"
...
for i in range(len(frequencies)):
    for j in range(len(supersample_factors)):
        case_file_name = 'mfg_t10_j1_f'+str(frequencies[i])+'_S'+str(supersample_factors[j])+'.sh'
        case_file = open(case_scripts_folder+case_file_name,'w')
        case_file.write(case_line1)
        case_file.write("#SBATCH --output=/home/coneill/sync/logs/mfg_t10_j1_f"+str(frequencies[i])+"_S"+str(supersample_factors[j])+"-%A.out")
        case_file.write("\n")
        case_file.write(case_line2)
        case_file.write("python /home/coneill/sync/code/pinns_data_assimilation/training_scripts/mazi_fixed_grid/test/mfg_t010_f001.py 1 "+str(frequencies[i])+" "+str(supersample_factors[j])+" 23\n")
