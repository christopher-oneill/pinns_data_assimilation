...
def boundary_points_function(cyl):
    ...
    boundary_tuple = (tf.cast(p_BC_vec,tf_dtype),tf.cast(cyl_BC_vec,tf_dtype)) # tf.cast(inlet_vec,tf_dtype)
...
def fit_epoch_pre(i_train,o_train,colloc_vector,boundary_tuple,ScalingParameters):
    ...
    for batch in range(batches):
        ...
        loss_value, data_loss, physics_loss, boundary_loss = train_step_pre(tf.cast(i_batch,tf_dtype),tf.cast(o_batch,tf_dtype),tf.cast(colloc_batch,tf_dtype),(tf.cast(BC_p_epoch,tf_dtype),tf.cast(BC_wall_batch,tf_dtype)),ScalingParameters) #
...
def fit_epoch(i_train,o_train,colloc_vector,boundary_tuple,ScalingParameters):
    ...
    for batch in range(batches):
        ...
        loss_value, data_loss, physics_loss, boundary_loss = train_step(tf.cast(i_batch,tf_dtype),tf.cast(o_batch,tf_dtype),tf.cast(colloc_batch,tf_dtype),(tf.cast(BC_p_epoch,tf_dtype),tf.cast(BC_wall_batch,tf_dtype)),ScalingParameters) #
        loss_vec[batch] = loss_value.numpy()
        data_vec[batch] = data_loss.numpy()
        physics_vec[batch] = physics_loss.numpy()
        boundary_vec[batch] = boundary_loss.numpy()
...
global job_name 
job_name = 'mf_dense007_{:03d}_S{:d}'.format(job_number,supersample_factor)
...
if supersample_factor>0:
    # copy the arrays if supersample factor is >0 so that the data size is approximately consistent
...
while pretrain_flag:
    ...
    ep_schedule = np.array([0,      50,     100,     200,       300,        400,         500])
...
while backprop_flag:
    ...
    ep_schedule = 500+np.array([0,    200,     400,   600  ])
