...
if __name__=="__main__":
    ...
    job_name = 'mf4d_f{:d}_S{:d}_j{:03d}'.format(mode_number,supersample_factor,job_number)
    ...
    embedding_wavenumber_vector = np.linspace(0,2*np.pi*ScalingParameters.MAX_x,200) # in normalized domain! in this case the wavenumber of the 3rd harmonic is roughly pi rad/s so we double that
    ...
    if len(model_file)>0:
        ...
        with tf.device(tf_device_string):        
            ...
            lo = QuadraticInputPassthroughLayer(300,2,activation='tanh',dtype=tf_dtype)(inputs)
            for i in range(3):
                lo = QuadraticInputPassthroughLayer(300,2,activation='tanh',dtype=tf_dtype)(lo)
    ...
    while backprop_flag:
        ...
        lr_schedule = np.array([1E-5, 3.16E-6, 1E-6,   3.16E-7, 1E-7,  0.0])
        ep_schedule = np.array([0,      50,     100,    400,    600,    800,])
