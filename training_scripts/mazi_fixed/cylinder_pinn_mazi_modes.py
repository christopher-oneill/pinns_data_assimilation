...
def net_f_cartesian(colloc_tensor):
    ...
    phi1x    = up[:,0]
    phi1y    = up[:,1] # note these are normalized by definition
    phi2x    = up[:,2]
    phi2y    = up[:,3] # note these are normalized by definition
    p1     = up[:,4]*MAX_p
    p2     = up[:,5]*MAX_p
    nut1   = up[:,6]*MAX_nut
    nut2   = up[:,7]*MAX_nut
    ...
    # phi1 gradients
    # Phi1 x gradient
    phi1x_x = tf.gradients(phi1x, colloc_tensor)[0][:,0]/MAX_x
    phi1x_y = tf.gradients(phi1x, colloc_tensor)[0][:,1]/MAX_y
    # and second derivative
    phi1x_xx = tf.gradients(phi1x_x, colloc_tensor)[0][:,0]/MAX_x
    phi1x_yy = tf.gradients(phi1x_y, colloc_tensor)[0][:,1]/MAX_y
    # uy gradient
    phi1y_x = tf.gradients(phi1y, colloc_tensor)[0][:,0]/MAX_x
    phi1y_y = tf.gradients(phi1y, colloc_tensor)[0][:,1]/MAX_y
    # and second derivative
    phi1y_xx = tf.gradients(phi1y_x, colloc_tensor)[0][:,0]/MAX_x
    phi1y_yy = tf.gradients(phi1y_y, colloc_tensor)[0][:,1]/MAX_y

    # Phi 2 gradients
    # Phi2 x gradient
    phi2x_x = tf.gradients(phi2x, colloc_tensor)[0][:,0]/MAX_x
    phi2x_y = tf.gradients(phi2x, colloc_tensor)[0][:,1]/MAX_y
    # and second derivative
    phi2x_xx = tf.gradients(phi2x_x, colloc_tensor)[0][:,0]/MAX_x
    phi2x_yy = tf.gradients(phi2x_y, colloc_tensor)[0][:,1]/MAX_y
    # uy gradient
    phi2y_x = tf.gradients(phi2y, colloc_tensor)[0][:,0]/MAX_x
    phi2y_y = tf.gradients(phi2y, colloc_tensor)[0][:,1]/MAX_y
    # and second derivative
    phi2y_xx = tf.gradients(phi2y_x, colloc_tensor)[0][:,0]/MAX_x
    phi2y_yy = tf.gradients(phi2y_y, colloc_tensor)[0][:,1]/MAX_y


    # prssure gradients
    p1_x = tf.gradients(p1, colloc_tensor)[0][:,0]/MAX_x
    p1_y = tf.gradients(p1, colloc_tensor)[0][:,1]/MAX_y
    p2_x = tf.gradients(p1, colloc_tensor)[0][:,0]/MAX_x
    p2_y = tf.gradients(p1, colloc_tensor)[0][:,1]/MAX_y

    # gradient nut
    nut1_x = tf.gradients(nut1, colloc_tensor)[0][:,0]/MAX_x
    nut1_y = tf.gradients(nut1, colloc_tensor)[0][:,1]/MAX_y
    nut2_x = tf.gradients(nut2, colloc_tensor)[0][:,0]/MAX_x
    nut2_y = tf.gradients(nut2, colloc_tensor)[0][:,1]/MAX_y
