...
# Case: Mazi Fixed
# 75 nodes wide 
# Now trying to train with 6 POD modes. now with a even more slowly increasing physics loss weight. No poisson equation. 
...
if node_name ==LOCAL_NODE:
    ...
    while True:
        ...
        if epochs >=40:
            physics_loss_coefficient=0.4*(24/3)
        if epochs >=60:
            physics_loss_coefficient=0.6*(24/3)
        if epochs >=80:
            physics_loss_coefficient=0.8*(24/3)
        if epochs>=50:
            
            keras.backend.set_value(model.optimizer.learning_rate, 0.005)
        if epochs>=100:
            physics_loss_coefficient=(24/3)
