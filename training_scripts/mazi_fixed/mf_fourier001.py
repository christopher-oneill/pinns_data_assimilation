...
phi_xr = np.array(fourierModeFile['velocityModes'][0,mode_number,:]).transpose()
...
MAX_uxppuxpp = np.max(uxux.flatten())
MAX_uxppuypp = np.max(uxuy.flatten())
MAX_uyppuypp = np.max(uyuy.flatten())
