...
def RANS_boundary_loss(model_RANS,ScalingParameters,boundary_tuple):
    (BC_p,BC_wall,BC_inlet_pts) = boundary_tuple
...
def boundary_points_function(cyl):
    ...
    boundary_tuple = (tf.cast(p_BC_vec,tf_dtype),tf.cast(cyl_BC_vec,tf_dtype),tf.cast(inlet_vec,tf_dtype)) # tf.cast(inlet_vec,tf_dtype)
...
def compute_loss_pre(x,y,colloc_x,boundary_tuple,ScalingParameters):
    ...
    physics_loss = tf.cast(0.0,tf_dtype)# RANS_physics_loss(model_RANS,ScalingParameters,colloc_x) 
...
def fit_epoch_pre(i_train,o_train,colloc_vector,boundary_tuple,ScalingParameters):
    ...
    (BC_p_epoch,BC_wall_epoch,BC_inlet_epoch) = boundary_tuple
    ...
    BC_inlet_rand  =tf.gather(BC_inlet_epoch,np.random.choice(np.arange(BC_inlet_epoch.shape[0]),batches*ScalingParameters.boundary_batch_size),axis=0)
    ...
    for batch in range(batches):
        ...
        BC_inlet_batch = BC_inlet_rand[(batch*ScalingParameters.boundary_batch_size):np.min([(batch+1)*ScalingParameters.boundary_batch_size,BC_inlet_rand.shape[0]]),:]

        loss_value, data_loss, physics_loss, boundary_loss = train_step(tf.cast(i_batch,tf_dtype),tf.cast(o_batch,tf_dtype),tf.cast(colloc_batch,tf_dtype),(tf.cast(BC_p_epoch,tf_dtype),tf.cast(BC_wall_batch,tf_dtype),tf.cast(BC_inlet_batch,tf_dtype)),ScalingParameters) #
...
global job_name 
job_name = 'mf_dense_boundary003_{:03d}_S{:d}'.format(job_number,supersample_factor)
...
inds_inlet_data = boundary_inds_irregular(600,200,X)
...
ScalingParameters.physics_loss_coefficient = tf.cast(3.16E-2,tf_dtype)
...
while backprop_flag:
    ...
    ep_schedule = np.array([0,    50,     100,    300,     500,    700  ])
    phys_schedule = np.array([3.16E-2, 3.16E-2, 3.16E-2, 3.16E-2, 3.16E-2, 3.16E-2, 3.16E-2])
