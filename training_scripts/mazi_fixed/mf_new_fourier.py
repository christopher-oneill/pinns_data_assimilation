...
# read the mode number from the command line
mode_number = int(sys.argv[1])
job_name = 'mf_new_fourier'+str(mode_number)+'_001'
...
grid_base_dir = HOMEDIR+'data/mazi_fixed_grid/'
...
configFileGrid = h5py.File(grid_base_dir+'configuration.mat','r')
...
with tf.device('/CPU:0'):
    model_mean = keras.models.load_model(HOMEDIR+'/output/mf_new_mean001_output/mf_new_mean001_ep58000_model.h5',custom_objects={'mean_loss':mean_loss_wrapper(f_colloc_train,ns_BC_vec,p_BC_vec),'QresBlock':QresBlock})
...
mean_data_test_grid = mean_cartesian(X_train_grid)
...
if True:
    ...
    while True:
        ...
        if np.mod(L_iter,1)==0:
            ...
            pred_grid = model_fourier.predict(X_train_grid,batch_size=32)
            ...
            if physics_loss_coefficient!=0:
                ...
                mxr_grid,mxi_grid,myr_grid,myi_grid,massr_grid,massi_grid = net_f_fourier_cartesian(X_train_grid,mean_data_test_grid)
