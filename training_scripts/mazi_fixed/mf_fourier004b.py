...
if __name__=="__main__":
    ...
    job_name = 'mf4b_f{:d}_S{:d}_j{:03d}'.format(mode_number,supersample_factor,job_number)
    ...
    ScalingParameters.boundary_batch_size = 64
    ...
    if len(model_file)>0:
        ...
        with tf.device(tf_device_string):        
            ...
            for i in range(4):                
                lo = QuadraticInputPassthroughLayer(120,2,activation='tanh',dtype=tf_dtype)(lo)
            ...
            for i in range(4):
                lo = QuadraticInputPassthroughLayer(200,2,activation='tanh',dtype=tf_dtype)(lo)
            # get the output
