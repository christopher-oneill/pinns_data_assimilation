...
def RANS_boundary_loss(model_RANS,ScalingParameters,boundary_tuple):
    (BC_p,BC_wall,BC_cylinder_inside_pts) = boundary_tuple # BC_inlet_pts
...
def fit_epoch(i_train,o_train,colloc_vector,boundary_tuple,ScalingParameters):
    ...
    for batch in range(batches):
        ...
        loss_value, data_loss, physics_loss, boundary_loss = train_step(tf.cast(i_batch,tf_dtype),tf.cast(o_batch,tf_dtype),tf.cast(colloc_batch,tf_dtype),boundary_tuple,ScalingParameters) #
...
global job_name 
job_name = 'mf_dense001_{:03d}_S{:d}'.format(job_number,supersample_factor)
...
if len(checkpoint_files)>0:
    ...
    with tf.device(tf_device_string):        
        ...
        lo = keras.layers.Dense(200,activation='tanh',dtype=tf_dtype)(inputs)
        for i in range(4):
            lo = keras.layers.Dense(200,activation='tanh',dtype=tf_dtype)(lo)
...
while backprop_flag:
    ...
    #ep_schedule = np.array([0,      50,     100,     150,    350,        450,    600])
    #phys_schedule = np.array([0.0, 1E-5, 1E-4, 1E-3, 1E-2, 1E-2, 1E-2])
    # regular training
