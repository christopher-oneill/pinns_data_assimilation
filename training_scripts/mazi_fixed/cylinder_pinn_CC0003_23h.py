...
if node_name==LOCAL_NODE:
    ...
    job_duration = timedelta(hours=23,minutes=30)
...
def custom_loss_wrapper(colloc_tensor_f): # def custom_loss_wrapper(colloc_tensor_f,BCs,BCs_p,BCs_t):
    ...
    def custom_loss(y_true, y_pred):
        ...
        physical_loss3 = tf.reduce_sum(tf.square(aloss))
                
        return data_loss_A1 + data_loss_A2 + data_loss_ux + data_loss_uy + data_loss_phi_1x + data_loss_phi_1y + data_loss_phi_2x + data_loss_phi_2y + data_loss_uxppuxpp + data_loss_uxppuypp +data_loss_uyppuypp + (11/3)*physical_loss1 + (11/3)*physical_loss2 + (11/3)*physical_loss3 # 0*f_boundary_p + f_boundary_t1+ f_boundary_t2 
