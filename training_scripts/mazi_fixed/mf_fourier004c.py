...
if __name__=="__main__":
    ...
    job_name = 'mf4c_f{:d}_S{:d}_j{:03d}'.format(mode_number,supersample_factor,job_number)
    ...
    embedding_wavenumber_vector = np.linspace(0,3*np.pi*ScalingParameters.MAX_x,120) # in normalized domain! in this case the wavenumber of the 3rd harmonic is roughly pi rad/s so we double that
    ...
    if len(model_file)>0:
        ...
        with tf.device(tf_device_string):        
            ...
            lo = InputPassthroughLayer(2,2,activation='linear')(inputs)
    ...
    while backprop_flag:
        ...
        #lr_schedule = np.array([3.3E-6, 1E-6,   3.3E-7, 1E-7,   3.3E-8, 0.0])
