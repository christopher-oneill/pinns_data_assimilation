...
def load_custom():
    ...
    model_RANS = keras.models.load_model(checkpoint_filename,custom_objects={'ResidualLayer':ResidualLayer,'CylindricalEmbeddingLayer':CylindricalEmbeddingLayer,'rbf_gaussian':rbf_gaussian})
    model_RANS.summary()
...
if True:
    ...
    with tf.device(tf_device_string):        
        ...
        lo = keras.layers.Dense(200,activation='linear',name='dense1')(inputs)
        lo = ResidualLayer(200,activation='swish',name='res1')(lo)
        lo = ResidualLayer(200,activation='swish',name='res2')(lo)
        lo = ResidualLayer(200,activation='swish',name='res3')(lo)
        lo = ResidualLayer(200,activation=rbf_gaussian,name='res4')(lo)
        lo = ResidualLayer(200,activation='swish',name='res5')(lo)
        lo = ResidualLayer(200,activation=rbf_gaussian,name='res6')(lo)
        lo = ResidualLayer(200,activation='swish',name='res7')(lo)
        lo = ResidualLayer(200,activation=rbf_gaussian,name='res8')(lo)
        lo = ResidualLayer(200,activation='swish',name='res9')(lo)
        lo = ResidualLayer(200,activation='swish',name='res10')(lo)
...
if True:
    ...
    lr_schedule = np.array([        3.3E-5,  1E-5,   3.3E-6,   1E-6,        1E-5,   3.3E-6,   1E-6,])
    ep_schedule = np.array([      0,       20,     70,      1030,         ])
    phys_schedule = np.array([      1E-30,   1E-30,  1E-30,    1E-30,    ])
    boundary_schedule = np.array([  0.01,  0.01,   0.01,     0.01,     ])
