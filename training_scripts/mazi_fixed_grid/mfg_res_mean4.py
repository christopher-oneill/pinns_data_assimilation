...
def load_custom():
    checkpoint_filename,training_steps = find_highest_numbered_file(savedir+'mfg_res_mean4_ep','[0-9]*','_model.h5')
...
global savedir
savedir = HOMEDIR+'output/mfg_res_mean4_010/'
...
def RANS_boundary_loss(model_RANS,ScalingParameters,boundary_tuple):
    ...
    mx,my,mass = RANS_reynolds_stress_cartesian2(model_RANS,ScalingParameters,BC_ns)
    ...
    boundary_loss = (BC_pressure_loss + BC_no_slip_loss + BC_inlet_loss+BC_inlet_loss2+BC_wall2)
...
def fit_epoch(i_train,o_train,colloc_points,boundary_tuple,ScalingParameters):
    ...
    if ScalingParameters.physics_loss_coefficient==0:
        colloc_points_sampled = np.zeros([i_train.shape[0],2],dtype=np.float64)
    else:
...
if True:
    ...
    lr_schedule = np.array([5E-5, 5E-5, 5E-5, 5E-5, 5E-5, 5E-5,])
    ep_schedule = np.array([0, 50, 150, 250, 350, 450,])
    phys_schedule = np.array([1E-6, 1E-5, 1E-4, 1E-3, 1E-2, 1E-2,])
