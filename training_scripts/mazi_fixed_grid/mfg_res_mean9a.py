...
def plot_err(epoch,model_RANS):
    ...
    # quantities
    for i in range(len(plot_save_exts)):
        plot.figure(epoch)
        #plot.title('Full Resolution')
        plot.subplot(3,1,1)
...
def save_custom():
    ...
    global model_RANS
    model_RANS.save(savedir+'mfg_res_mean9a_ep'+str(np.uint(training_steps))+'_model.h5')
    pred = model_RANS(i_test,training=False)
    h5f = h5py.File(savedir+'mfg_res_mean9a_ep'+str(np.uint(training_steps))+'_pred.mat','w')
    ...
    if ScalingParameters.physics_loss_coefficient!=0:
        ...
        h5f = h5py.File(savedir+'mfg_res_mean9a_ep'+str(np.uint(training_steps))+'_error.mat','w')
...
def load_custom():
    checkpoint_filename,training_steps = find_highest_numbered_file(savedir+'mfg_res_mean9a_ep','[0-9]*','_model.h5')
...
global savedir
savedir = HOMEDIR+'output/mfg_res_mean9a_001/'
...
ScalingParameters.colloc_batch_size = 30000
...
def train_step(x,y,colloc_x,boundary_tuple,ScalingParameters):
    with tf.GradientTape() as tape:
        ...
        data_loss = tf.reduce_sum(tf.reduce_mean(tf.square(y_pred[:,0:5]-y),0),0)
...
if True:
    ...
    ep_schedule = np.array([0, 1000, 2000, 3000, 4000])
