...
case_prefix_list = ['mfg_boundary_test_','mfg_boundary_dense_','mfg_boundary_qres_']#['mfg_dense_large','mfg_dense_small','mfg_qres_large','mfg_qres_small']
...
for case_prefix in case_prefix_list:
    for m in range(len(supersample_factor_list)):
        ...
        training_cases = extract_matching_integers(output_base_dir+case_prefix,'[0-9][0-9][0-9]','_S'+str(supersample_factor))


        for k in training_cases:
            case_name = case_prefix + "{:03d}".format(k)
            print('This case is: ', case_name)
            output_dir = output_base_dir+case_name + '_S'+str(supersample_factor)+'/'
            ...
            if predfilename == None:
                print('Nothing found for '+case_name)
                continue
            ...
            errorfilename = output_dir+case_name+'_S'+str(supersample_factor)+'_ep'+str(epoch_number)+'_error.mat'
            print(errorfilename)
    ...
    # check if the error file exists, if so plot
            largefilename = output_dir+case_name+'_S'+str(supersample_factor)+'_ep'+str(epoch_number)+'_pred_large.mat'
            print(largefilename)
