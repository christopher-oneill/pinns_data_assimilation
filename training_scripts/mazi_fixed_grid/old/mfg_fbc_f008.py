...
def compute_loss(x,y,colloc_x,mean_grads,boundary_tuple,ScalingParameters):
    ...
    physics_loss=tf.cast(1E-30,tf_dtype) #physics_loss = FANS_physics_loss(model_FANS,colloc_x,mean_grads,ScalingParameters) 
    boundary_loss=tf.cast(1E-30,tf_dtype)# boundary_loss = FANS_boundary_loss(model_FANS,boundary_tuple,ScalingParameters)
...
if __name__=="__main__":
    ...
    job_name = 'mfg_fbcf008_f{:d}_S{:d}_j{:03d}'.format(mode_number,supersample_factor,job_number)
    ...
    embedding_wavenumber_vector = np.linspace(0,24*np.pi*ScalingParameters.MAX_x,480) # in normalized domain! in this case the wavenumber of the 3rd harmonic is roughly pi rad/s so we double that
    ...
    if model_file is not None:
        ...
        with tf.device(tf_device_string):        
            ...
            lo = QuadraticInputPassthroughLayer(50,2,activation='tanh',dtype=tf_dtype)(inputs)
            for i in range(4):
                lo = QuadraticInputPassthroughLayer(50,2,activation='tanh',dtype=tf_dtype)(lo)
