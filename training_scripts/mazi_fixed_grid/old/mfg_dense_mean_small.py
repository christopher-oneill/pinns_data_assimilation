...
def RANS_boundary_loss(model_RANS,ScalingParameters,boundary_tuple):
    ...
    boundary_loss = (BC_wall_loss + BC_pressure_loss + BC_cylinder_inside_loss) 
...
def colloc_points_function(a,b):
    ...
    colloc_limits2 = np.array([[-1.0,2.0],[-1.0,1.0]])
...
def boundary_points_function(cyl,inside):
    ...
    boundary_tuple = (p_BC_vec,cyl_BC_vec,cylinder_inside_vec)
...
boundary_tuple = boundary_points_function(1080,500)
global colloc_vector
colloc_vector = colloc_points_function(40000,10000)
