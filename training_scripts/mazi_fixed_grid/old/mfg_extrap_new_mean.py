...
job_name = 'mfg_extrap_new_mean{:03d}_S{:d}'.format(job_number,supersample_factor)
...
x = np.array(configFile['X_vec'][0,:]) # vector for each training data
x_grid = np.array(configFile['x_grid']) # vector for unique training datas
y = np.array(configFile['X_vec'][1,:])
y_grid = np.array(configFile['y_grid'])
x_test = x # the test set should be the whole domain
y_test = y
inds_wake = x>0.5
inds_wake_grid = x_grid>0.5
...
# now that we have the normalized values, crop to the wake
x = x[inds_wake]
y = y[inds_wake]
x_grid = x_grid[inds_wake_grid]
ux = ux[inds_wake]
uy = uy[inds_wake]
uxppuxpp = uxppuxpp[inds_wake]
uxppuypp = uxppuypp[inds_wake]
uyppuypp = uyppuypp[inds_wake]
