...
def fit_epoch(i_train,o_train,colloc_points,boundary_tuple,ScalingParameters):
    ...
    for batch in range(batches):
        ...
        loss_value,data_loss,physics_loss,boundary_loss = train_step(tf.cast(i_batch,tf.float64),tf.cast(o_batch,tf.float64),[],boundary_tuple,ScalingParameters) #tf.cast(tf.concat((i_batch,colloc_batch),axis=0),tf.float64)
...
if False:
    ...
    ep_schedule = np.array([        0,       50,     100,      2000,   3000, ])
    ...
    while True:
        ...
        if (np.mod(training_steps,100)==0):
