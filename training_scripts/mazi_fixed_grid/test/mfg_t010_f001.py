...
def FANS_boundary_loss(model_FANS,boundary_tuple,ScalingParameters):
    ...
    loss_no_slip = FANS_BC_no_slip(model_FANS,pts_no_slip) # no no-slip, seems to increase pressure error
...
if __name__=="__main__":
    ...
    job_name = 'mfg_t010_f001_f{:d}_S{:d}_j{:03d}'.format(mode_number,supersample_factor,job_number)
    ...
    from pinns_data_assimilation.lib.layers import FourierEmbeddingLayer as CosEmbeddingLayer
    from pinns_data_assimilation.lib.layers import FourierLayer
    # load the saved mean model

    with tf.device('/CPU:0'):
        mean_model_filename,mean_training_steps = find_highest_numbered_file(PROJECTDIR+'output/mfg_t010_001_S'+str(supersample_factor)+'/mfg_t010_001_S'+str(supersample_factor)+'_ep','[0-9]*','_model.h5')
        model_mean = keras.models.load_model(mean_model_filename,custom_objects={})
    ...
    if model_file is not None:
        with tf.device(tf_device_string):
            ...
            model_FANS.save(savedir+job_name+'_ep'+str(training_steps)+'_model.h5')
            exit()
    else:
        # create a new model
        training_steps = 0
