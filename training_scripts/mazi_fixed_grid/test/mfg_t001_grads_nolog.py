...
def plot_gradients():
    ...
    for i in range(data_grads.shape[2]):
        ...
        plot.colorbar()
        if saveFig:
            plot.savefig(fig_dir+'ep'+str(np.uint(training_steps))+'_gradients_'+labels[i]+'.png',dpi=300)
        plot.close(1)
    ...
    plot.colorbar()
    if saveFig:
        plot.savefig(fig_dir+'ep'+str(np.uint(training_steps))+'_NS_residual_finite_difference.png',dpi=300)
    plot.close(1)
    ...
    plot.colorbar()
    if saveFig:
        plot.savefig(fig_dir+'ep'+str(np.uint(training_steps))+'_NS_residual_finite_difference_log.png',dpi=300)
    plot.close(1)
...
def plot_NS_residual():
    ...
    plot.colorbar()
    if saveFig:
        plot.savefig(fig_dir+'ep'+str(np.uint(training_steps))+'_NS_residual.png',dpi=300)
...
def plot_err():
    ...
    for i in range(6):
        ...
        plot.colorbar()
        if saveFig:
            plot.savefig(fig_dir+'ep'+str(np.uint(training_steps))+plot_save_exts[i],dpi=300)
        plot.close(1)
    ...
    for i in range(6):
        ...
        plot.xlim(-2,10)
        plot.savefig(fig_dir+'ep'+str(np.uint(training_steps))+plot_save_exts2[i],dpi=300)
        plot.close(1)
...
def load_custom():
    ...
    model_RANS.summary()
    print('Model Loaded. Epoch',str(np.uint(training_steps)))
...
def compute_loss(x,y,colloc_x,boundary_tuple,ScalingParameters):
    ...
    return total_loss, tf.cast(data_loss,tf_dtype), physics_loss, boundary_loss, (1+log_data)*data_loss, (1+log_physics)*physics_loss, (1+log_boundary)*boundary_loss

# define the training functions
@tf.function
def train_step(x,y,colloc_x,boundary_tuple,ScalingParameters):
    with tf.GradientTape() as tape:
        total_loss ,data_loss, physics_loss, boundary_loss = compute_loss(x,y,colloc_x,boundary_tuple,ScalingParameters)[0:4]
...
def fit_epoch(i_train,o_train,colloc_vector,boundary_tuple,ScalingParameters):
    ...
    print('Epoch',str(np.uint(training_steps)),f" Loss: {np.mean(loss_vec):.6e}",f" Data loss: {np.mean(data_vec):.6e}")
...
def train_LBFGS(model,x,y,colloc_x,boundary_tuple,ScalingParameters):
    ...
    def f(params_1d):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            loss_value, data, physics, boundary, wdata, wphysics, wboundary = compute_loss(x,y,colloc_x,boundary_tuple,ScalingParameters)
        
        # save the loss gradients for analysis
        grads_data = tape.gradient(data, model.trainable_variables)
        temp_grads_data = tf.reshape(grads_data[0],(tf.math.reduce_prod(grads_data[0].shape),))
        for i in range(1,len(grads_data)):
            temp_grads_data = tf.concat((temp_grads_data,tf.reshape(grads_data[i],(tf.math.reduce_prod(grads_data[i].shape),))),axis=0)
        tf.py_function(f.grads_history_data.append, inp=[temp_grads_data], Tout=[])
        
        grads_wdata = tape.gradient(wdata, model.trainable_variables)
        temp_grads_wdata = tf.reshape(grads_wdata[0],(tf.math.reduce_prod(grads_wdata[0].shape),))
        for i in range(1,len(grads_wdata)):
            temp_grads_wdata = tf.concat((temp_grads_data,tf.reshape(grads_wdata[i],(tf.math.reduce_prod(grads_wdata[i].shape),))),axis=0)
        tf.py_function(f.grads_history_wdata.append, inp=[temp_grads_wdata], Tout=[])
        
        grads_physics = tape.gradient(physics, model.trainable_variables)
        temp_grads_physics = tf.reshape(grads_physics[0],(tf.math.reduce_prod(grads_physics[0].shape),))
        for i in range(1,len(grads_physics)):
            temp_grads_physics = tf.concat((temp_grads_data,tf.reshape(grads_physics[i],(tf.math.reduce_prod(grads_physics[i].shape),))),axis=0)
        tf.py_function(f.grads_history_physics.append, inp=[temp_grads_physics], Tout=[])

        grads_wphysics = tape.gradient(wphysics, model.trainable_variables)
        temp_grads_wphysics = tf.reshape(grads_wphysics[0],(tf.math.reduce_prod(grads_wphysics[0].shape),))
        for i in range(1,len(grads_wphysics)):
            temp_grads_wphysics = tf.concat((temp_grads_data,tf.reshape(grads_wphysics[i],(tf.math.reduce_prod(grads_wphysics[i].shape),))),axis=0)
        tf.py_function(f.grads_history_wphysics.append, inp=[temp_grads_wphysics], Tout=[])

        grads_boundary = tape.gradient(boundary, model.trainable_variables)
        temp_grads_boundary = tf.reshape(grads_boundary[0],(tf.math.reduce_prod(grads_boundary[0].shape),))
        for i in range(1,len(grads_boundary)):
            temp_grads_boundary = tf.concat((temp_grads_data,tf.reshape(grads_boundary[i],(tf.math.reduce_prod(grads_boundary[i].shape),))),axis=0)
        tf.py_function(f.grads_history_boundary.append, inp=[temp_grads_boundary], Tout=[])

        grads_wboundary = tape.gradient(wboundary, model.trainable_variables)
        temp_grads_wboundary = tf.reshape(grads_wboundary[0],(tf.math.reduce_prod(grads_wboundary[0].shape),))
        for i in range(1,len(grads_wboundary)):
            temp_grads_wboundary = tf.concat((temp_grads_data,tf.reshape(grads_wboundary[i],(tf.math.reduce_prod(grads_wboundary[i].shape),))),axis=0)
        tf.py_function(f.grads_history_wboundary.append, inp=[temp_grads_wboundary], Tout=[])
        ...
        tf.py_function(f.history_data.append, inp=[data], Tout=[])
        tf.py_function(f.history_physics.append, inp=[physics], Tout=[])
        tf.py_function(f.history_boundary.append, inp=[boundary], Tout=[])
    ...
    f.history_physics = []
    f.history_boundary = []
    f.grads_history_data = []
    f.grads_history_wdata = []
    f.grads_history_physics = []
    f.grads_history_wphysics = []
    f.grads_history_boundary = []
    f.grads_history_wboundary = []

    return f


def save_gradients(func,epoch):
    h5f = h5py.File(savedir+job_name+'_ep'+str(np.uint(training_steps))+'_grads.h5','w')
    h5f.create_dataset('loss_history',data=np.array(func.history))
    h5f.create_dataset('data_loss_history',data=np.array(func.history_data))
    h5f.create_dataset('physics_loss_history',data=np.array(func.history_physics))
    h5f.create_dataset('boundary_loss_history',data=np.array(func.history_boundary))
    h5f.create_dataset('grads_data',data=np.array(func.grads_history_data))
    h5f.create_dataset('grads_wdata',data=np.array(func.grads_history_wdata))
    h5f.create_dataset('grads_physics',data=np.array(func.grads_history_physics))
    h5f.create_dataset('grads_wphysics',data=np.array(func.grads_history_wphysics))
    h5f.create_dataset('grads_boundary',data=np.array(func.grads_history_boundary))
    h5f.create_dataset('grads_wboundary',data=np.array(func.grads_history_wboundary))
    h5f.close()
...
global job_name 
job_name = 'mfg_t001_grads_nolog_{:03d}_S{:d}'.format(job_number,supersample_factor)
...
n_colloc=15000
n_boundary = 720
boundary_tuple = boundary_points_function(n_boundary)
colloc_vector = colloc_points_function(n_colloc)
...
if True:
    ...
    L_iter = 0
    boundary_tuple = boundary_points_function(n_boundary)
    colloc_vector = colloc_points_function(n_colloc) # one A100 max = 60k?
    ...
    start_train_steps = training_steps
    ...
    while True:
        ...
        training_steps = start_train_steps + func.iter # since func.iter is only reset every 10 L steps, we just add it to the outside counter
        L_iter = L_iter+1
        # we need to save every step here so that we don't OOM
        save_gradients(func,training_steps) # save loss and gradients before resetting the optimizer
        start_train_steps = training_steps # update the outside train step counter only every time we reset func 
        func = train_LBFGS(model_RANS,tf.cast(i_train_LBFGS,tf_dtype),tf.cast(o_train_LBFGS,tf_dtype),colloc_vector,boundary_tuple,ScalingParameters)
