...
if len(checkpoint_files)>0:
    ...
    model_fourier = keras.models.load_model(PROJECTDIR+'output/'+job_name+'_output/'+job_name+'_ep'+str(epochs)+'_model.h5',custom_objects={'custom_loss':fourier_loss_wrapper(f_colloc_train,mean_data,ns_BC_vec,p_BC_vec,inlet_BC_vec),'FourierResidualLayer':FourierResidualLayer,'ResidualLayer':ResidualLayer})
    ...
    if useGPU:
        ...
        with strategy.scope():
            ...
            model_fourier.add(FourierResidualLayer(fourier_nodes))
            for i in range(fourier_layers-2):
