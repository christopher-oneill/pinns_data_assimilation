...
def fit_epoch(i_train,o_train,colloc_points,boundary_tuple,ScalingParameters):
    ...
    for batch in range(batches):
        ...
        loss_value, data_loss, physics_loss, boundary_loss = train_step(tf.cast(i_batch,tf.float64),tf.cast(o_batch,tf.float64),tf.cast(tf.concat((i_batch,colloc_batch),axis=0),tf.float64),sample_boundary(ScalingParameters,boundary_tuple),ScalingParameters) #
...
ScalingParameters.data_loss_coefficient=100.0
ScalingParameters.batch_size=i_train.shape[0]
ScalingParameters.boundary_batch_size=20000
ScalingParameters.colloc_batch_size=25000
history_list = []
...
if True:
    ...
    lr_schedule = np.array([        1E-4,  3.3E-5,  1E-5,   3.3E-6,  3.3E-6,        ])
    ep_schedule = np.array([        0,     50,      100,    150,     200,         ])
    phys_schedule = np.array([      1E-1,   1E-1,   1E-1,   1E-1,    1E-1,       ])
    boundary_schedule = np.array([  100.0,  100.0,      100.0,    100.0,     100.0,       ])
