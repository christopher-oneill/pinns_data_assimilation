...
def save_custom():
    ...
    global model_RANS
    model_RANS.save(savedir+'mfg_res_mean7b_ep'+str(np.uint(training_steps))+'_model.h5')
    pred = model_RANS(i_test,training=False)
    h5f = h5py.File(savedir+'mfg_res_mean7b_ep'+str(np.uint(training_steps))+'_pred.mat','w')
    ...
    if ScalingParameters.physics_loss_coefficient!=0:
        ...
        h5f = h5py.File(savedir+'mfg_res_mean7b_ep'+str(np.uint(training_steps))+'_error.mat','w')
...
def load_custom():
    checkpoint_filename,training_steps = find_highest_numbered_file(savedir+'mfg_res_mean7b_ep','[0-9]*','_model.h5')
...
global savedir
savedir = HOMEDIR+'output/mfg_res_mean7b_001/'
...
if True:
    ...
    lr_schedule = np.array([1E-4, 1E-4, 3.3E-5, 1E-5, 3.3E-6, 1E-6, 3.3E-6])
    ep_schedule = np.array([0, 30, 200, 400, 600, 800])
    phys_schedule = np.array([1E-6, 1E0, 1E0, 1E0, 1E0, 1E0, 1E0,])
