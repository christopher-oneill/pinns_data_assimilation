...
def save_custom():
    ...
    global model_RANS
    global trial_name
    model_RANS.save(savedir+trial_name+'_ep'+str(np.uint(training_steps))+'_model.h5')
    pred = model_RANS(i_test,training=False)
    h5f = h5py.File(savedir+trial_name+'_ep'+str(np.uint(training_steps))+'_pred.mat','w')
    ...
    if ScalingParameters.physics_loss_coefficient!=0:
        ...
        h5f = h5py.File(savedir+trial_name+'_ep'+str(np.uint(training_steps))+'_error.mat','w')
...
def load_custom():
    global trial_name
    checkpoint_filename,training_steps = find_highest_numbered_file(savedir+trial_name+'_ep','[0-9]*','_model.h5')
...
global savedir
global trial_name
trial_name = 'mfg_res_mean7c_002'
savedir = HOMEDIR+'output/'+trial_name+'/'
...
def RANS_reynolds_stress_cartesian2(model_RANS,ScalingParameters,colloc_tensor):
    ...
    f_mass = ux_x + uy_y
    f_c = tf.zeros_like(f_x,dtype=tf.float64)#tf.abs(-cx)+tf.abs(-cy) # we should have no momentum transfer to the solid inside the fluid
...
def RANS_boundary_loss(model_RANS,ScalingParameters,boundary_tuple):
    ...
    BC_cylinder_inside_loss = 1000.0*BC_cylinder_inside(model_RANS,ScalingParameters,BC_cylinder_inside_pts)
...
def RANS_error_gradient2(model_RANS,ScalingParameters,colloc_tensor):
    ...
    f_y = (ux*uy_x + uy*uy_y) + (uxuy_x + uyuy_y) + p_y - (ScalingParameters.nu_mol)*(uy_xx+uy_yy)#+ uxuy_x + uyuy_y    #- nu*(ux_xx+ur_xr+ur_x/r)
    f_m = ux_x + uy_y
    f_c = tf.cast(0.0,tf.float64)#tf.abs(-cx)+tf.abs(-cy) # we should have no momentum transfer to the solid inside the fluid
...
if True:
    training_steps = 0
    nodes = np.int64(np.concatenate((np.linspace(10,200,10),np.array([200,200,200])),axis=0))
    with tf.device(tf_device_string):
        
        inputs = keras.Input(shape=(2,),name='coordinates')
        layer_o = ResidualLayer(nodes[0],activation='tanh',name='res'+str(0))(inputs)
        for i in range(1,nodes.shape[0]):
            layer_o = ResidualLayer(nodes[i],activation='tanh',name='res'+str(i))(layer_o)
        outputs = keras.layers.Dense(8,activation='linear',name='dynamical_quantities')(layer_o)
...
def train_step(x,y,colloc_x,boundary_tuple,ScalingParameters):
    with tf.GradientTape() as tape:
        ...
        data_loss = tf.reduce_sum(tf.reduce_mean(tf.square(y_pred[:,0:5]-y[:,0:5]),0),0)
...
def fit_epoch(i_train,o_train,colloc_points,boundary_tuple,ScalingParameters):
    ...
    # randomly sample the collocation points
    colloc_rand_indices = np.random.randint(0,colloc_points.shape[0]-1,batches*ScalingParameters.colloc_batch_size)
    colloc_points_sampled = colloc_points[colloc_rand_indices,:]

    # randomly shuffle the data
    shuffle_inds = np.random.shuffle(np.arange(0,i_train.shape[0]))
    i_sampled = i_train[shuffle_inds,:][0,:,:]
    o_sampled = o_train[shuffle_inds,:][0,:,:]
...
if True:
    ...
    ep_schedule = np.array([0, 500, 1000, 1500, 2000,])
