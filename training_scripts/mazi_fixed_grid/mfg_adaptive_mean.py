...
def net_f_cartesian(colloc_tensor):
    ...
    # unknowns
    p = up[:,5]*MAX_p
    w = tf.square(up[:,6])+1.0 # self adaptive weight function must be minimum 1
...
def mean_loss_wrapper(colloc_tensor_f,BC_ns,BC_p): # def custom_loss_wrapper(colloc_tensor_f,BCs,BCs_p,BCs_t):
    ...
    def mean_loss(y_true, y_pred):
        ...
        if physics_loss_coefficient!=0:
            ...
            w_data = tf.square(y_pred[:,6])+1.0 # the weight must be minimum 1
            ...
            adaptive_loss = 3.0/(tf.reduce_mean(w_phys)+tf.reduce_mean(w_data)+tf.reduce_min(w_phys)+tf.reduce_min(w_data)) # SA pinn adaptive weight function loss
