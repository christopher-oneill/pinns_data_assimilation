...
def plot_err():
    ...
    for i in range(6):
        ...
        if supersample_factor>1:
            if i<5:
                plot.scatter(i_train_plot[:,0],i_train_plot[:,1],1,'k','.')
        plot.subplot(3,1,2)
...
ScalingParameters.batch_size=32
ScalingParameters.boundary_batch_size=128
ScalingParameters.colloc_batch_size=512 # note that because we use grad and randomized collocs the actual NS computations is (batch_size+2*colloc_batch_size)
history_list = []
...
if True:
    ...
    lr_schedule = np.array([         3.3E-5,  1E-5,   3.3E-6,  3.3E-6,        ])
    ep_schedule = np.array([        0,     100,      300,    600,          ])
    phys_schedule = np.array([       1E0,   1E0,   1E0,    1E0,       ])
    boundary_schedule = np.array([   1.0,      1.0,    1.0,     1.0,       ])
