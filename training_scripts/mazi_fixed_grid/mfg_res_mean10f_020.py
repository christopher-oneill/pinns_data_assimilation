...
def train_step(x,y,colloc_x,boundary_tuple,ScalingParameters):
    with tf.GradientTape() as tape:
        total_loss = compute_loss_backprop(x,y,colloc_x,boundary_tuple,ScalingParameters)
...
def fit_epoch(i_train,o_train,colloc_points,boundary_tuple,ScalingParameters):
    ...
    print('Epoch',str(training_steps),f" Total loss Value: {np.mean(loss_vec):.6e}",)
...
if True:
    ...
    with tf.device(tf_device_string):        
        ...
        lo = ResidualLayer(150,activation='swish')(lo)
        lo = ResidualLayer(150,activation='swish')(lo)
        lo = ResidualLayer(120,activation='swish')(lo)
        lo = ResidualLayer(90,activation='swish')(lo)
        lo = ResidualLayer(60,activation='swish')(lo)
        lo = ResidualLayer(30,activation='swish')(lo)
        lo = ResidualLayer(10,activation='swish')(lo)
...
ScalingParameters.colloc_batch_size=8192
history_list = []
...
if True:
    ...
    ep_schedule = np.array([        0,     50,      150,    250,     350,     550,      750,    ])
