...
if True:
    ...
    ep_schedule = supersample_factor*supersample_factor*np.array([        0,     50,      150,    250,     350,         ])
    phys_schedule = np.array([      1E-3,   1E-3,   1E-2,   1E-2,    1E-2,       ])
    ...
    while True:
        for i in range(1,len(ep_schedule)):
            if training_steps==ep_schedule[i]:
                ...
                #save_custom()           
        if (np.mod(training_steps,10*supersample_factor*supersample_factor)==0):    
            batches = np.int64(np.ceil(i_train.shape[0]/(1.0*ScalingParameters.batch_size)))
            colloc_sampled = RANS_sample_by_grad_err(model_RANS,colloc_vector,ScalingParameters,batches*ScalingParameters.colloc_batch_size)
        fit_epoch(i_train,o_train,colloc_sampled,boundary_tuple,ScalingParameters)
        if (np.mod(training_steps,50*supersample_factor*supersample_factor)==0):
            plot_large()
            plot_NS_large()
        ...
        if (np.mod(training_steps,supersample_factor*supersample_factor)==0):
            plot_NS_residual() 
            plot_err()
