...
def save_custom():
    ...
    global model_RANS
    model_RANS.save(savedir+'mfg_res_mean8_ep'+str(np.uint(training_steps))+'_model.h5')
    pred = model_RANS(i_test,training=False)
    h5f = h5py.File(savedir+'mfg_res_mean8_ep'+str(np.uint(training_steps))+'_pred.mat','w')
    ...
    if ScalingParameters.physics_loss_coefficient!=0:
        ...
        h5f = h5py.File(savedir+'mfg_res_mean8_ep'+str(np.uint(training_steps))+'_error.mat','w')
...
def load_custom():
    checkpoint_filename,training_steps = find_highest_numbered_file(savedir+'mfg_res_mean8_ep','[0-9]*','_model.h5')
    model_RANS = keras.models.load_model(checkpoint_filename,custom_objects={'ResidualLayer':ResidualLayer,'CylindricalEmbeddingLayer':CylindricalEmbeddingLayer,'FourierEmbeddingLayer':FourierEmbeddingLayer})
    model_RANS.summary()
...
global savedir
savedir = HOMEDIR+'output/mfg_res_mean8_003/'
...
cylinder_inds = np.less(np.power(np.power(cylinder_inside_vec[:,0]-c1_loc[0],2)+np.power(cylinder_inside_vec[:,1]-c1_loc[1],2),0.5),0.5*d)
cylinder_inside_vec = np.delete(cylinder_inside_vec,cylinder_inds[0,:],axis=0)
...
frequency_vector = np.linspace(0,30,60)
if False:
    training_steps = 0
    nodes = 100
    with tf.device(tf_device_string):
        ...
        femb = FourierEmbeddingLayer(frequency_vector)(inputs)
        x = keras.layers.Dense(nodes,activation='linear',name='dense1')(femb)
        for i in range(10):
...
def train_step(x,y,colloc_x,boundary_tuple,ScalingParameters):
    with tf.GradientTape() as tape:
        ...
        dynamic_data_weight = tf.math.exp(tf.math.ceil(tf.math.log(combined_physics_loss+1E-30)))
        total_loss = dynamic_data_weight*ScalingParameters.data_loss_coefficient*data_loss + combined_physics_loss
...
if True:
    ...
    lr_schedule = np.array([1E-4, 3.3E-5, 1E-5, 3.3E-6, 1E-6, 5E-6])
    ep_schedule = np.array([0, 100, 200, 300, 400, 500])
    phys_schedule = np.array([1E0, 1E0, 1E0, 1E0, 1E0, 1E0, 1E0])
