...
def compute_loss(x,y,colloc_x,boundary_tuple,ScalingParameters):
    ...
    physics_loss = tf.cast(1.0E-30,tf_dtype)#RANS_physics_loss(model_RANS,ScalingParameters,colloc_x) #tf.cast(0.0,tf_dtype)#
    boundary_loss = tf.cast(1.0E-30,tf_dtype)#RANS_boundary_loss(model_RANS,ScalingParameters,boundary_tuple)
...
global job_name 
job_name = 'mfg_fbc001_{:03d}_S{:d}'.format(job_number,supersample_factor)
