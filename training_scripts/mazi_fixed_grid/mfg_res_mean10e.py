...
def plot_err(epoch,model_RANS):
    ...
    mx,my,mass = RANS_reynolds_stress_cartesian(model_RANS,ScalingParameters,i_test[:])
    ...
    if False:
        ...
        plot.colorbar()
        if saveFig:
            plot.savefig(fig_dir+'ep'+str(epoch)+'_p_x.png',dpi=300)
        plot.close(1)
        ...
        plot.colorbar()
        if saveFig:
            plot.savefig(fig_dir+'ep'+str(epoch)+'_p_y.png',dpi=300)
        plot.close(1)
    ...
    # quantities
    for i in range(6):
        o_test_min = np.nanmin(o_test_grid_temp[:,:,i].ravel())
        o_test_max = np.nanmax(o_test_grid_temp[:,:,i].ravel())
        o_test_level = np.max([abs(o_test_min),abs(o_test_max)])
        o_test_levels = np.linspace(-o_test_level,o_test_level,21)
        plot.figure(1)
        plot.title('Full Resolution')
        plot.subplot(3,1,1)
        ...
        plot.colorbar()
        plot.subplot(3,1,2)
        p_test_min = np.nanmin(pred_test_grid[:,:,i].ravel())
        p_test_max = np.nanmax(pred_test_grid[:,:,i].ravel())
...
inlet_BC_x = -10.0*np.ones([1000,1])/ScalingParameters.MAX_x
inlet_BC_y = np.linspace(-10.0,10.0,1000)/ScalingParameters.MAX_y
...
boundary_tuple = (p_BC_vec,cyl_BC_vec,inlet_BC_vec)
...
@tf.function
def RANS_pressure_gradients(model_RANs,colloc_tensor):
    up = model_RANS(colloc_tensor)
    p = up[:,5]*ScalingParameters.MAX_p
...
def RANS_boundary_loss(model_RANS,ScalingParameters,boundary_tuple):
    (BC_p,BC_wall,BC_inlet) = boundary_tuple
    # outlet pressure
    ...
    boundary_loss = (BC_wall_loss + BC_pressure_loss + BC_inlet_loss ) #  + BC_wall2 +   + BC_cylinder_inside_loss + BC_inlet_loss2
...
def fit_epoch(i_train,o_train,colloc_points,boundary_tuple,ScalingParameters):
    ...
    colloc_sampled = RANS_sample_by_grad_err(model_RANS,colloc_points,ScalingParameters,batches*ScalingParameters.colloc_batch_size)
    ...
    for batch in range(batches):
        ...
        colloc_batch = colloc_sampled[(batch*ScalingParameters.colloc_batch_size):np.min([(batch+1)*ScalingParameters.colloc_batch_size,colloc_sampled.shape[0]]),:]
...
if False:
    ...
    ep_schedule = np.array([        0,       80,     480,       720,   1000, ])
    ...
    ScalingParameters.physics_loss_coefficient=1E0
    ScalingParameters.boundary_loss_coefficient=1E0
    # LBFGS
    import tensorflow_probability as tfp
    L_iter = 0
    colloc_vector = colloc_points_function(5000,15000,5000)
