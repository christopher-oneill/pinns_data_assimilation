...
for m in range(len(supersample_factor_list)):
    ...
    for k in training_cases:
        ...
        if os.path.isfile(errorfilename):
            ...
            weight = np.array(errorFile['weight'])
            ...
            weight_grid = weight.reshape(X_grid.shape)
            weight_grid[cylinder_mask]=np.NaN
            ...
            ax.set_xlim(left=-2.0,right=10.0)
            ax.set_ylim(bottom=-2.0,top=2.0)
            fig7.add_subplot(4,1,4)
            plot.contourf(X_grid,Y_grid,np.log10(weight_grid),levels=21)
            plot.set_cmap('bwr')
            plot.colorbar()
            plot.ylabel('y/D')
            plot.xlabel('x/D')
