...
for k in training_cases:
    ...
    meanVelocityFile = h5py.File(data_dir+'meanVelocityS64.mat','r')
    configFile = h5py.File(data_dir+'configurationS64.mat','r')
    meanPressureFile = h5py.File(data_dir+'meanPressureS64.mat','r')
    reynoldsStressFile = h5py.File(data_dir+'reynoldsStressS64.mat','r')
    ...
    p_pred_grid = np.reshape(p_pred,X_grid.shape) -np.mean(p_pred)+np.mean(p)
