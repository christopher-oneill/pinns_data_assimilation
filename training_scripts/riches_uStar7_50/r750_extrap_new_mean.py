...
job_name = 'r750_extrap_new_mean{:03d}_S{:d}'.format(job_number,supersample_factor)
...
x_test = x # use the full resolution
y = np.array(configFile['X_vec'][1,:])
y_test = y
d = np.array(configFile['cylinderDiameter'])
# set the cylinder location
c1_loc = np.array([-0.014-0.5*d,-0.5*d],dtype=np.float64)
...
def colloc_points():
    ...
    # min(X)=-0.014, max(X)=0.1205
    # min(Y)=-0.0712, max(Y)=0.05144
    # I think the cylinder center is located -0.5 diameters in y relative to the coordinate system
    # and -0.5 diameters in x relative to the left _edge_


    colloc_limits1 = np.array([[-0.014-3.5*d,0.12],[-0.071,0.051]])
    colloc_sample_lhs1 = LHS(xlimits=colloc_limits1)
    colloc_lhs1 = colloc_sample_lhs1(80000)


    colloc_limits2 = np.array([[-0.014-1.5*d,(3-0.5)*d-0.014],[-2.5*d,1.5*d]])
...
ns_BC_x = (0.5*d*np.cos(theta)+c1_loc[0])/MAX_x # we beed to normalize the boundary conditions as well
ns_BC_y = (0.5*d*np.sin(theta)+c1_loc[1])/MAX_y
