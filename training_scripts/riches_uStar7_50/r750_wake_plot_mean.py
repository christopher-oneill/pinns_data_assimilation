...
data_dir = 'C:/projects/pinns_narval/sync/data/riches_uStar7_50/'
case_prefix = 'r750_wake_new_mean'
...
for m in range(len(supersample_factor_list)):
    ...
    for k in training_cases:
        ...
        MAX_p= 2*0.5*1000*0.2215*0.2215/1000 # estimated maximum pressure
        ...
        cylinder_mask = (np.power(np.power(X_grid,2)+np.power(Y_grid,2),0.5)<(0.0*d))
        ...
        plot.contourf(X_grid,Y_grid,(ux_pred_grid-ux_grid)/MAX_ux,levels=50)
        plot.set_cmap('bwr')
        plot.colorbar()
        plot.ylabel('y/D')
        plot.xlabel('x/D')
