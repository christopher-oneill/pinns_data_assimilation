...
snap_inds = np.array([100,500,1000,1500,2000,2500,3000,3500])

for sn in range(snap_inds.size):
    figures_folder = output_base_dir+'figures/'
    create_directory_if_not_exists(figures_folder)
    figure_prefix = figures_folder + '_reconstr'
    ...
    plot.axis('equal')
    plot.contourf(X_grid,Y_grid,velocityFieldGrid[:,:,snap_inds[sn],0],levels=f1_levels)
    plot.set_cmap('bwr')
    plot.colorbar()
    ax=plot.gca()
    ...
    ax.set_xlim(left=x_lim_vec[0],right=x_lim_vec[1])
    ax.set_ylim(bottom=y_lim_vec[0],top=y_lim_vec[1])
    plot.savefig(figure_prefix+'_ux_sn'+str(snap_inds[sn])+'.png',dpi=300)
    plot.close()
