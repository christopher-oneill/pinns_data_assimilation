...
if node_name ==LOCAL_NODE:
    ...
    if True:
        ...
        while True:
            ...
            func.assign_new_model_parameters(results.position)
            init_params = tf.dynamic_stitch(func.idx, model.trainable_variables) # we need to reasign the parameters otherwise we start from the beginning each time
            epochs = epochs +100
            L_iter = L_iter+1
            ...
            if np.mod(L_iter,10)==0:
                model.save(save_loc+job_name+'_ep'+str(np.uint(epochs))+'_model.h5')
                pred = model.predict(X_train,batch_size=32)
                h5f = h5py.File(save_loc+job_name+'_ep'+str(np.uint(epochs))+'_pred.mat','w')
                h5f.create_dataset('pred',data=pred)
                h5f.close()
