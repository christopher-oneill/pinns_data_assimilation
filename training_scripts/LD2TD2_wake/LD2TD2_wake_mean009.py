...
def net_f_cartesian(colloc_tensor):
    ...
    # gradient unmodel_meaned reynolds stresses
...
if len(checkpoint_files)>0:
    ...
    model_mean = keras.models.load_model(HOMEDIR+'/output/'+job_name+'_output/'+job_name+'_ep'+str(epochs)+'_model.h5',custom_objects={'custom_loss':custom_loss_wrapper(f_colloc_train),})
else:
    ...
    if useGPU:
        ...
        with strategy.scope():
            ...
            model_mean.summary()
            model_mean.compile(optimizer=keras.optimizers.SGD(learning_rate=0.01), loss = custom_loss_wrapper(tf.cast(f_colloc_train,dtype_train)),jit_compile=False) #(...,BC_points1,...,BC_points3)
...
if node_name ==LOCAL_NODE:
    ...
    hist = model_mean.fit(temp_X_train[0,:,:],temp_Y_train[0,:,:], batch_size=32, epochs=d_epochs, callbacks=[early_stop_callback,checkpoint_callback])
else:
    ...
    if True:
        ...
        func = function_factory(model_mean, custom_loss_wrapper(f_colloc_train), X_train, O_train)
    ...
    while False:
        ...
        if epochs>50:
            keras.backend.set_value(model_mean.optimizer.learning_rate, 5E-4)
        if epochs>100:
